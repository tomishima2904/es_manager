...
public interface EntrysheetsRepository extends JpaRepository<EntrysheetsEntity, EntrysheetsKey> {
    public List<EntrysheetsEntity> findByUserId(String userId);

    public EntrysheetsEntity findByUserIdAndEsId(String userId, int esId);

    @Query(value = "SELECT MAX(es_id) FROM entrysheets WHERE user_id = :userId", nativeQuery = true)
    public int findMaxEsIdByUserId(@Param("userId") String userId);
}
