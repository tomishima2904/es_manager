...
public class QuestionsController {
    ...
    @PostConstruct
    public void init(){
        QuestionsEntity question1 = new QuestionsEntity(0L, 0L, 0, "志望動機は?", 0, "ホワイトだから");
        QuestionsEntity question2 = new QuestionsEntity(0L, 0L, 1, "趣味は?", 1, "サウナ");
        QuestionsEntity question3 = new QuestionsEntity(0L, 0L, 0, "志望動機は?", 1, "駅近だから");
        QuestionsEntity question4 = new QuestionsEntity(0L, 1L, 0, "ガクチカは?", 0, "特になし");
    ...
    public Mono<List<QuestionsEntity>> getEntrysheetQuestions(@PathVariable Long userId, @PathVariable Long esId) {

        // 条件に合うuserIdのあるesIdのデータをデータベースから取得
        List<QuestionsEntity> questions = questionsRepository.findByUserIdAndEsId(userId, esId);
        // List<QuestionsEntity> questions = questionsRepository.findByEsId(esId);
